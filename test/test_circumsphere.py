...
from NucFrames.depth_utils.circumsphere import _circumsphere, _circumcircle
...
def regular_tetrahedron_coords(radius):
  ...
  return (np.array([[1 / 3 * np.sqrt(3) * a, 0, 0], [-1 / 6 * np.sqrt(3) * a, 0.5 * a, 0],
                    [-1 / 6 * np.sqrt(3) * a, -0.5 * a, 0], [0, 0, 1 / 3 * np.sqrt(6) * a]],
                   np.float64))
